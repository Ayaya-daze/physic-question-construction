...
当前仓库的 Codex Skill/Agent Inbox 是开发维护和人工批次修复工具。比赛运行目标由项目自行完成页级调度、选择性视觉 API、审核闸门和原子物化，不要求安装 Codex Agent 或完整第三方 OCR 系统。
...
- Word / JSON：转换成题目文件；复杂字段可合并进正文或答案。
...
即可完成校验和安全物化。若输出含 `human_review_needed: true`，任务会停在 `needs_review`；人工对照来源确认后，显式增加 `--approve-review` 才允许提交、重建索引和归档。
...
- 原始 `source.pdf` 可以保留在独立来源存储中作为审计材料，但不能进入题目 `assets/`；组卷导出不能靠拼接原始 PDF 代替题目文本渲染。
...
- 生成的题目正文和 Markdown/LaTeX 预览。
- 生成的答案正文和预览。
- 候选题图资产。
- 可选元数据和 warnings。
- 编辑题目、编辑答案、确认 warning、批准和驳回。
