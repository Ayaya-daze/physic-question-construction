...
扫描 PDF 的生产导入路径应使用 vision-capable LLM 或 Codex `physics-question-importer` skill 输出结构化文件题目，再通过 `/api/file-questions/import` 导入 JSON 和独立图片资产。Text-only DeepSeek/CnOcr 只能作为草稿辅助，不能作为扫描卷生产真源。
...
- 后续可接真实 embedding 模型或外部向量库，但向量库仍只是可重建索引。
